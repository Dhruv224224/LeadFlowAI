...
export default function TrustBadges() {
  ...
  return (
    <div className="relative py-10 border-y border-white/5 overflow-hidden">
      ...
      <motion.div
        className="flex gap-12 animate-scroll-x will-change-transform"
        style={{ width: 'max-content' }}
      >
        {row.map((b, i) => (
          <div
            key={i}
            className="flex items-center gap-3 text-text-main/40 hover:text-light-blue transition-colors shrink-0"
          >
